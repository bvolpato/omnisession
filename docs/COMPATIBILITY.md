...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Codex | >= 0.146.0 | `~/.codex/sessions/**/*.jsonl` | `codex fork ID` | Linux, macOS, Windows | Linux, macOS, Windows | Linux, macOS, Windows | Linux, macOS | Minimum-version app-server session import | Linux: source-ci + installed-token-free<br>macOS: source-ci + synthetic-store<br>Windows: source-ci + synthetic-store + installed-token-free |
...
Windows packaging, installer, CLI, shims, and synthetic stores are source-tested in Windows CI. Codex and Grok declare Windows read/index, clean start, and same-provider resume, while only Grok declares cross-provider import: launch tests start their npm command shims through `node.exe`, never `cmd.exe`, in ordinary workspace paths, and installed releases run token-free import and read-back in Windows CI. During a Windows native import, the first Ctrl+C or Ctrl+Break only requests a stop. Import helpers run in their own console process group and finish publishing, then omni rolls the generated session back exactly, undoes recorded lineage, and exits 130 without launching; a second Ctrl+C exits at once. Ctrl+Break typed at the console also reaches helpers and can end an import before it reports a target to roll back. Codex therefore stays undeclared for Windows cross-provider import, so automatic picker and shim routing use semantic handoff; explicit `omni resume --in codex` still attempts the runtime-validated native import. Other picker targets without declared cross-provider import also continue through semantic handoff. Installed OpenCode import runs token-free Windows conformance but stays undeclared; its adapter refuses npm `.cmd` launchers it cannot route. Explicit targets use provider-specific binary, version, schema, rollback, and read-back gates; manifest capability absence means not declared, not necessarily impossible. Source CI is not evidence of installed-provider compatibility. WSL is a separate Linux environment and uses the Linux installer; native Windows and WSL provider stores are not treated as interchangeable.
