...
Windows packaging, installer, CLI, shims, and synthetic stores are source-tested in Windows CI. Codex and Grok declare Windows read/index, clean start, same-provider resume, and cross-provider import: installed releases run token-free native import and read-back through `omni` in Windows CI, and launch tests start their npm command shims through `node.exe`, never `cmd.exe`, in ordinary workspace paths. Installed OpenCode import also runs token-free Windows conformance but stays undeclared; its adapter refuses npm `.cmd` launchers it cannot route. Explicit targets use provider-specific binary, version, schema, rollback, and read-back gates; manifest capability absence means not declared, not necessarily impossible. Source CI is not evidence of installed-provider compatibility. WSL is a separate Linux environment and uses the Linux installer; native Windows and WSL provider stores are not treated as interchangeable.
