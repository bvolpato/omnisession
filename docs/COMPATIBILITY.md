...
Run `omni adapters` for concise status or `omni --json adapters` for structured fields. Output separates declared platform capability, detected source and launcher, available transfer route, and runtime validation still required. Discovery reads paths and bounded static metadata; it does not launch provider or desktop applications. `omni adapters --check-imports` opts into running each installed provider's version command and reports per provider whether the native import version gate passes (`native_import_check` in JSON), or the reason a transfer would fall back to semantic handoff. A declared route is not a completed transfer proof: version, schema, active-writer, rollback, and target read-back gates run only after user requests transfer.
