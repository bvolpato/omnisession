...
| OpenCode | local `0.0.0-bv/opencode-queue-202607230403` | Official list/export CLI | `opencode --session ID --fork` | Official export preferred |
...
Compatibility is capability-based. v0.1 canonicalizes recognized visible records and omits unrecognized private records. Malformed records are skipped. Native provider files are opened read-only where possible and never rewritten by OmniSession.
