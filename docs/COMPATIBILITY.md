...
| Grok | >= 0.2.114 | `~/.grok/sessions/*/*/` | `grok --resume ID --fork-session` | Linux, macOS, Windows | Linux, macOS, Windows | Linux, macOS, Windows | Linux, macOS | Minimum-version ACP import and read-back | Linux: source-ci + installed-token-free<br>macOS: source-ci + synthetic-store<br>Windows: source-ci + synthetic-store + installed-token-free |
...
Windows packaging, installer, CLI, shims, and synthetic stores are source-tested in Windows CI. Codex and Grok declare Windows read/index, clean start, and same-provider resume: launch tests start their npm command shims through `node.exe`, never `cmd.exe`, in ordinary workspace paths. Their cross-provider import stays undeclared on Windows because a console Ctrl+C there cannot yet roll back a native import, so shim routing into them uses semantic handoff. On Windows, picker targets without declared cross-provider import continue through semantic handoff. Explicit `omni resume --in` still attempts native import, and installed releases run token-free import and read-back through it in Windows CI. Installed OpenCode import also runs token-free Windows conformance but stays undeclared; its adapter refuses npm `.cmd` launchers it cannot route. Explicit targets use provider-specific binary, version, schema, rollback, and read-back gates; manifest capability absence means not declared, not necessarily impossible. Source CI is not evidence of installed-provider compatibility. WSL is a separate Linux environment and uses the Linux installer; native Windows and WSL provider stores are not treated as interchangeable.
