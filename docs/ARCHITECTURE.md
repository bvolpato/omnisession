...
1. `AdapterRegistry` asks each adapter to probe its installation and list sessions. Listing returns metadata such as IDs, titles, workspaces, branches, and timestamps. Most adapters read native stores directly; OpenCode lists and exports through its official CLI.
...
7. **Interrupt safely.** On Linux and macOS, the first `Ctrl+C` from materialization until launch only sets a flag. Checkpoints after read-back, launch planning, and lineage recording roll back the target and exit with status 130 without launching. Shim-routed imports and Windows keep the default `Ctrl+C` action for now.
