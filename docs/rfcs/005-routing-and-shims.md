...
Transparent routing is opt-in. `omni shim install --bin-dir DIR` installs `claude`, `codex`, `opencode`, `grok`, `agy`, `pi`, and `cursor-agent` symlinks in `${OMNISESSION_HOME:-$HOME/.omnisession}/shims`. `DIR/omni` must already exist and be executable. Install never overwrites an unowned path. It prints shell-specific PATH guidance; shim directory must precede real provider directories. `omni shim uninstall --bin-dir DIR` removes only symlinks targeting that exact OmniSession binary.
...
`OMNI_BYPASS=1` always bypasses routing. Real binary resolution excludes shim directory and current `omni` executable, preventing PATH recursion. Absolute override variables are `OMNI_CLAUDE_BIN`, `OMNI_CODEX_BIN`, `OMNI_OPENCODE_BIN`, `OMNI_GROK_BIN`, `OMNI_ANTIGRAVITY_BIN`, `OMNI_PI_BIN`, and `OMNI_CURSOR_AGENT_BIN`. Final launch uses process replacement on Unix, preserving stdin, stdout, stderr, signals, and TTY behavior.
