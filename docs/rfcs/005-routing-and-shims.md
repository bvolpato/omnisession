...
Transparent routing is opt-in. `omni shim install --bin-dir DIR` installs provider aliases in `${OMNISESSION_HOME:-$HOME/.omnisession}/shims`. Unix aliases are `claude`, `codex`, `opencode`, `grok`, `hermes`, `agy`, `pi`, and `cursor-agent` symlinks. Windows aliases use the same command names with `.exe` and are hard links to compiled `omni.exe`; `.cmd` wrappers are never created. `agent` is Cursor's primary launcher, but OmniSession never installs that collision-prone alias. Cursor Agent discovery prefers an `agent` path whose canonical target has Cursor Agent identity and falls back to `cursor-agent`. Cursor IDE discovery and diagnostics use executable names and static product metadata only. They never execute desktop binary; only explicit resume or import run path launches it. Verified older desktop builds and exact unversioned `Cursor.AppImage` installs support only explicit same-provider workspace open; cross-provider import readiness and target pickers require minimum supported version. `DIR/omni` or `DIR/omni.exe` must already exist and be executable. Install never overwrites an unowned path. It prints platform-specific PATH guidance; shim directory must precede real provider directories. `omni shim uninstall --bin-dir DIR` removes only symlinks or hard links owned by that exact OmniSession binary, plus Windows aliases left on an older OmniSession build. Replacing `omni.exe` leaves Windows hard links on old build. Install relinks such an alias only when it is a regular file whose content is a PE image carrying OmniSession alias-creation marker text; it never executes alias. Relink renames old alias to a retired sibling, which works even while alias runs, then moves a staged hard link into its name. If any alias change fails, install renames every retired link back, newest first, removes aliases it created, and names each alias it could not restore. After success, retired links are deleted, or by a later install once a running alias exits. Windows installer runs `omni shim install` after an upgrade when any alias exists. Hard links cannot cross volumes.
