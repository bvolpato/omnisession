...
Cursor IDE 3.12.17 commit `0fb762053c34788bb7760d5673f8a6d4c8589d50` is accepted as a Linux-only private SQLite writer. Support requires an exact AppImage or installed-bundle fingerprint plus the accepted `composerHeaders` and `cursorDiskKV` schema fingerprint. Linux workspace identity follows Cursor's path-and-inode MD5 contract, so a target does not need an existing workspace-storage record. OmniSession requires Cursor to be closed, begins an immediate transaction with no busy wait, inserts only a new composer UUID and its new conversation keys, then reads every generated message through the independent Cursor IDE adapter. It never reads or copies `ItemTable`, which can contain authentication state. Rollback compares and deletes only exact generated rows. Cursor has no verified command-line route to one IDE chat, so launch opens the workspace and leaves exact selection to History.
