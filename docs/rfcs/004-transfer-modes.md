...
OmniSession implements native resume, OpenCode CLI import, version-gated Codex app-server injection, Grok ACP import, exact-version Claude JSONL materialization, semantic handoff, and portable export. Native target imports preserve visible user and assistant history plus bounded tool records as documentary messages. Later conformance reports expand to files, subagents, permissions, checkpoints, plans, and attachments. Status is `preserved`, `summarized`, `historical_only`, `redacted`, `omitted`, or `unsupported`.
