...
- Ask instead of quietly writing a handoff file when a native import cannot run. On a terminal OmniSession says why and offers to retry (after closing the agent that blocked the import, for example), fork in the source agent instead, continue with a handoff file, or cancel; routed shim commands offer retry, handoff, or cancel. A failed Cursor IDE import, which has no handoff, offers retry, fork in the source agent, or cancel, and a failed same-agent fork offers retry or cancel, where both used to stop. Forking in the source agent starts it in its own default permission mode, never the one chosen for the target. An empty line asks again, and cancel still reports the failure. Scripts, pipes, `--json`, and `--dry-run` keep the automatic fallback. ([#153](https://github.com/bvolpato/omnisession/pull/153))
...
- Fall back to semantic handoff when `opencode import` fails before creating a session, instead of aborting with "rollback also failed" because OpenCode refuses to delete a session that does not exist. The rollback is still attempted, and a failed delete counts as a rollback failure whenever the target can be read. ([#150](https://github.com/bvolpato/omnisession/pull/150))
...
- Drop the alpha label from the README and website. ([#142](https://github.com/bvolpato/omnisession/pull/142))
...
- List Codex sessions faster in `omni list`, `omni search`, `omni index`, and the session picker: each rollout's session metadata is cached under `OMNISESSION_HOME`, and only new or changed rollouts are read again. ([#144](https://github.com/bvolpato/omnisession/pull/144))
